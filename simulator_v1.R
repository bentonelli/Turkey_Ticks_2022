...
for (n in 1:num_sims){
  ...
  br_larvae_molt <- rnorm(1,0,.001)
  br_nymph_molt <- rnorm(1,0,.001)
  ...
  exp_temp_param_larv_molt <- rtruncnorm(1,a=0,mean=.0001,sd=.0003)
  exp_temp_param_nymph_molt <- rtruncnorm(1,a=0,mean=.0001,sd=.0003)
  exp_precip_param_larv_molt <- rnorm(1,mean=.0005,sd=.0005)
  exp_precip_param_nymph_molt <- rnorm(1,mean=.0005,sd=.0005)
  exp_temp_param_adult_breed <- rnorm(1,mean=.0005,sd=.0005)
  
  nymph_start <- rtruncnorm(1,a=0,mean=50,sd=50)
  adult_start <- rtruncnorm(1,a=0,mean=10,sd=10)
  
  start_date <- 1 ## Keep this at 1 for now
  ...
  sim_run <- turkey_tick_model_v1(temp_data = temp_data_avg,precip_data = precip_data_avg,
                                  ...
                                  starting_nymphs = starting_nymphs,
